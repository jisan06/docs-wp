 ...
 */
class ActivityLogger extends Library\ObjectAbstract implements ActivityLoggerInterface
{
    /**
    ...
    {
        $config->append([
            'actions'      => ['after.edit', 'after.add', 'after.delete'],
            'title_column' => ['title', 'name'],
    ...
    public function getActivityStatus(Library\ModelEntityInterface $object, $action = null)
    {
        ...
        if ($action == 'after.add' && $status == Library\ModelEntityInterface::STATUS_UPDATED) {
            $status = Library\ModelEntityInterface::STATUS_CREATED;
        }
     ...
     */
    public function getActivityData(Library\ModelEntityInterface $object, Library\ObjectIdentifierInterface $subject, $action = null)
    {
        $data = [
