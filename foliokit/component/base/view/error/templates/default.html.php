...
<?= helper('debug.debugger') ?>

<title content="replace"><?= translate('Error').' '.$code.' - '. EasyDocLabs\Library\HttpResponse::$status_messages[$code]; ?></title>
        ...
        <div id="the_error">
            <div class="error_container">
                <div class="error_container__header">
                    <?= helper('debug.path', ['file' => $file]) ?>:<span class="linenumber"><?= $line ?></span>
                </div>
                <div class="error_container_code">
                    <?= helper('debug.source', ['file' => $file, 'line' => $line]) ?>
                </div>
            </div>
        </div>
    </div>
    <div class="page_content">
        <? $trace_steps = helper('debug.trace', ['trace' => $trace]); ?>
        ...
        <div id="codes_container" class="codes_container" style="counter-reset: source-counter <?= count($trace_steps)+1; ?>">
            <? foreach (helper('debug.trace', ['trace' => $trace]) as $i => $step): ?>
                <? if ($step['file']): $source_id = 'source'.$i; ?>
                    <div id="<?= $source_id ?>" class="codes_container__item">
                        <div class="codes_container__content">
                            ...
                            <div class="error_container">
                                <div class="error_container__header">
                                    <span class="file"><?= helper('debug.path', ['file' => $step['file']]) ?></span>:<span class="linenumber"><?= $step['line'] ?></span>
                                </div>
                                <? if (isset($source_id)): ?>
                                        ...
                                        <table cellspacing="0">
                                            <? foreach ($step['args'] as $name => $arg): ?>
                                                <tr>
                                                    ...
                                                    <td><pre class="arguments"><?= helper('debug.dump', ['value' => $arg, 'object_depth' => $i ? 1 : 4]) ?></pre></td>
                                                </tr>
                                            <? endforeach ?>
                                        </table>
                                    </div>
