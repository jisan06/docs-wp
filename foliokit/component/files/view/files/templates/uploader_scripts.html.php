...
$server_limit = \EasyDocLabs\Component\Files\ModelEntityContainer::getServerUploadLimit(); ?>
...
<div class="k-dynamic-content-holder">
    ...
    <script data-inline type="text/html" class="js-uploader-template" data-name="upload-pending">
        <div>
            <?= translate('{total} files in the queue', [
                'total' => '{{=it.total}}'
            ]) ?>
    ...
    <script data-inline type="text/html" class="js-uploader-template" data-name="uploading">
        <div>
            <?= translate('Uploading {total} files, {remaining} to go', [
                'total' => '{{=it.total}}',
                'remaining' => '{{=it.remaining}}'
            ], '</span>') ?>
            {{? it.failed > 0 }}
            -
            <?= translate('{failed} errors', [
                'failed' => '{{=it.failed}}'
            ]) ?>
            {{?}}
            <a class="k-upload__details-button js-open-info">
    ...
    <script data-inline type="text/html" class="js-uploader-template" data-name="upload-finished">
        <div>
            <?= translate('Uploaded {total} files', [
                'total' => '{{=it.total}}'
            ]) ?>
            {{? it.failed > 0 }}
            -
