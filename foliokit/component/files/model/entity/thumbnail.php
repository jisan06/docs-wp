 ...
 */
class ModelEntityThumbnail extends ModelEntityFile
{
    /**
    ...
    protected function _initialize(Library\ObjectConfig $config)
    {
        $config->append([
            ...
            'data'      => ['crop' => false, 'dimension' => ['width' => 200, 'height' => 150]]
        ]);
    ...
    public function setAdapter()
    {
        ...
        $this->_adapter = $this->getObject('com:files.adapter.file', ['path' => $path]);
        ...
        if ($this->invokeCommand('before.save', $context) !== false)
        {
            if ($source = $this->source)
            {
                if ($str = $this->generate())
                {
                    ...
                    $folder = $this->getObject('com:files.adapter.folder', ['path' => $path]);
    ...
    {
        if (!$value instanceof ModelEntityFile || !$value instanceof ModelEntityFiles) {
