...
class DatabaseBehaviorSluggable extends DatabaseBehaviorAbstract
{
    ...
    protected function _beforeInsert(DatabaseContextInterface $context)
    {
        ...
        // Push filter to the table for later use when inserting
        $table->getColumn('slug', true)->filter = $this->_createFilter();
    ...
    protected function _createFilter()
    {
        if (!$this->_filter instanceof FilterInterface)
        {
            ...
            //Create the filter
            $this->_filter = $this->getObject('filter.factory')->createChain($this->_filter, $config);
        }
    ...
    protected function _canonicalizeSlug()
    {
        ...
        $query = $table->getDriver()->getQuery('select');
