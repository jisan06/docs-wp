...
class DatabaseDriverMysqli extends DatabaseDriverAbstract
{
     ...
     */
     public function connect()
     {
        ...
        if (mysqli_connect_errno()) {
            throw new DatabaseException('Connect failed: (' . mysqli_connect_errno() . ') ' . mysqli_connect_error(), mysqli_connect_errno());
        }
    ...
    public function setDatabase($database)
    {
        if(!$this->_connection->select_db($database)) {
            throw new DatabaseException('Could not connect with database : '.$database);
        }
    ...
    protected function _parseTableInfo($info)
    {
        ...
        $table->type        = isset($info->Comment) && $info->Comment == 'VIEW' ? 'VIEW' : 'BASE';
        $table->length      = $info->Data_length;
        $table->autoinc     = $info->Auto_increment;
        $table->collation   = $info->Collation;
        $table->behaviors   = array();
        $table->description = isset($info->Comment) && $info->Comment != 'VIEW' ? $info->Comment : '';
