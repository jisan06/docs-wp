 ...
 * @package EasyDocLabs\Library\Database\Table
 */
abstract class DatabaseTableAbstract extends ObjectAbstract implements DatabaseTableInterface, ObjectMultiton
{
     ...
     * @throws DatabaseException If the table does not exist.
     */
    public function __construct(ObjectConfig $config)
    {
        ...
        if (!$info = $this->getSchema()) {
            throw new DatabaseException('Table ' . $this->_name . ' does not exist');
        }
    ...
    {
        // Create query object.
        $query = $this->getObject('lib:database.query.delete', ['driver' => $this->getDriver()])
            ->table($this->getBase());
        ...
        foreach ($data as $key => $value)
        {
            ...
            if ($filter = $column->filter)
            {
                if (!$filter instanceof FilterInterface) {
                    $filter = $this->getObject('filter.factory')->createChain($column->filter);
                }
