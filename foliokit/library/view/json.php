...
class ViewJson extends ViewAbstract
{
    ...
    protected function _actionRender(ViewContext $context)
    {
        ...
        if (is_array($content) || $content instanceof \Traversable) {
            $content = new ObjectConfigJson($content);
        }
    ...
    protected function _getUserLinks(ModelEntityInterface $user)
    {
        return ['self' => sprintf('/wp-json/wp/v2/users/%s', $user->id)]; 
    }
     ...
     * @return array
     */
    protected function _includeResource(ModelEntityInterface $entity, $properties = [], $cache = true)
    {
        if (empty($properties))
        {
            $properties = [
        ...
        if ($cache)
        {
            $cache = $entity->getIdentifier()->name.'-'.md5(json_encode($entity->toArray()));
        ...
        else $resource = $this->_createResource($entity);

        $resource = array_intersect_key($resource, array_flip($properties));
     ...
     * @return array
     */
    protected function _includeCollection(ModelEntityInterface $entities, $properties = [], $cache = true)
    {
        $result = [];

        foreach ($entities as $entity) {
            $result[] = $this->_includeResource($entity, $properties, $cache);
        }
     ...
     */
    protected function _callCustomMethod(ModelEntityInterface $entity, $suffix)
    {
        $result = false;
        $name   = StringInflector::singularize($entity->getIdentifier()->name);
        $method = '_get'.ucfirst($name).ucfirst($suffix);

        if ($method !== '_getEntity'.ucfirst($suffix) && method_exists($this, $method)) {
