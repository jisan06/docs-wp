...
class TemplateHelperBehavior extends TemplateHelperAbstract
{
     ...
     * @return string
     */
    public function foliokit($config = array())
    {
        ...
        if (!static::isLoaded('foliokit'))
        {
            $html .= $this->jquery($config);
            $html .= $this->buildElement('ktml:script', ['src' => 'assets://js/foliokit'.($config->debug ? '' : '.min').'.js']);

            static::setLoaded('foliokit');
        }
    ...
    public function vue($config = array())
    {
        ...
        if ($config->entity instanceof ModelEntityInterface)
        {
            ...
            $html .= $this->foliokit($config);
            $html .= $this->_onDomReady("
                var form = $('.k-js-form-controller');
                
                if (form.length) {
                    form.data('controller').store = Foliokit.EntityStore.create({
                        form: form,
    ...
    /**
     * Loads the Forms.Validator class and connects it to Foliokit.Controller.Form
     *
     * @param array|ObjectConfig $config
     * @return string   The html output
     */
    ...
    public function select2($config = array())
    {
        ...
        {
            $html .= $this->jquery($config);
            $html .= $this->buildElement('ktml:script', ['src' => 'assets://js/foliokit.select2'.($config->debug ? '' : '.min').'.js']);
    ...
    public function autocomplete($config = array())
    {
        ...
        if($config->element && !static::isLoaded($signature))
        {
            ...
            $html .= $this->_onDomReady('var selector = $("'.$config->element.'");
                    selector.select2(Foliokit.getSelect2Options('.$options.'));
                    '.$init."\n");
    ...
    /**
     * Loads the Foliokit customized jQtree behavior and renders a sidebar-nav list useful in split views
     *
    ...
    public function tree($config = array())
    {
        ...
        {
            $html .= $this->foliokit();
            $html .= $this->buildElement('ktml:script', ['src' => 'assets://js/foliokit.tree'.($config->debug ? '' : '.min').'.js']);
        ...
        if($config->element && !static::isLoaded($signature))
        {
            ...
            $html .= $this->_onDomReady('new Foliokit.Tree('.json_encode($config->element).', '.$options.');');
