...
class DispatcherBehaviorLimitable extends DispatcherBehaviorAbstract
{
    ...
    protected function _beforeInclude(DispatcherContext $context)
    {
        ...
        $limit = $controller->getRequest()->getQuery()->get('limit', 'int');

        $max = $this->getConfig()->max ?? $this->getConfig()->default;

        if (!$limit || $limit > $max) $this->_beforeGet($context); // Apply limitations
    }
}
