...
abstract class ControllerModel extends ControllerView implements ControllerModellable
{
    ...
    protected function _actionEdit(ControllerContext $context)
    {
        ...
        if(count($entities))
        {
            ...
            {
                if ($entities->getStatus() === Database::STATUS_FAILED)
                {
                    $error = $entities->getStatusMessage();
                    throw new ControllerExceptionActionFailed($error ? $error : 'Edit Action Failed');
                }
            }
            else $context->response->setStatus(HttpResponse::RESET_CONTENT); //Only set the reset content status if the action explicitly succeeded
        }
