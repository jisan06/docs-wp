...
class FilesystemLocatorFactory extends ObjectAbstract implements ObjectMultiton
{
    ...
    public function registerLocator($identifier)
    {
        ...
        if(!$class || !array_key_exists(__NAMESPACE__.'\FilesystemLocatorInterface', class_implements($class)))
        {
            throw new \UnexpectedValueException(
                'Locator: '.$identifier.' does not implement FilesystemLocatorInterface'
            );
        }
