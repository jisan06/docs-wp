<?php
if ( !class_exists('Puc_easydoc_Plugin_UpdateChecker', false) ):
	 ...
	 * @access public
	 */
	class Puc_easydoc_Plugin_UpdateChecker extends Puc_easydoc_UpdateChecker {
	    const URL_PREFIX = 'easydoclabs=1&plugin=';
...
//		protected $updateTransient = 'update_plugins';
		...
		public function __construct($metadataUrl, $pluginFile, $slug = '', $checkPeriod = 12, $optionName = '', $muPluginFile = ''){
			$this->pluginAbsolutePath = wp_normalize_path($pluginFile);
            ...
            $extensions = 'easydoc@'.(isset($plugin_data['Version']) ? $plugin_data['Version'] : 'unknown');

            $this->addQueryArgFilter(function($args) use($extensions) {
                global $wp_version;

                $args['extensions'] = $extensions;
                $args['php'] = phpversion();
                $args['wordpress'] = $wp_version;
            ...
            $this->addHttpRequestArgFilter(function($args) {
            ...
            $this->addUpgraderDownloadFilter();
        }

        public function beforeDownload($requestArgs, $url)
        {
            return $requestArgs;
        }

        public function getApiKey() {
		    return get_option('easydocs_api_key');
        }

        public function getSiteKey() {
		    return get_option('easydocs_site_key');
        }

		public function addUpgraderDownloadFilter()
        {
            add_filter('upgrader_pre_download', function($reply, $package, $upgrader) {
                static $filterAdded = false;

                $needle = static::URL_PREFIX.$this->slug;

                if (!$filterAdded && strpos($package, $needle) !== false) {
                    add_filter('http_request_args', function($requestArgs, $url = '') use($needle) {

                        if (strpos($url, $needle) !== false) {
                            $requestArgs['headers']['Referer'] = site_url();

                            if ($api_key = $this->getApiKey()) {
                                $requestArgs['headers']['Authorization'] = 'Bearer '.$api_key;
                            }

                            $requestArgs = $this->beforeDownload($requestArgs, $url);
                        }
		 ...
		 * @return Puc_easydoc_Scheduler
		 */
		protected function createScheduler($checkPeriod) {
			$scheduler = new Puc_easydoc_Scheduler($this, $checkPeriod, ['load-plugins.php']);
...
//			remove_action('admin_init', [$this, 'handleManualCheck']);
		 ...
		 * @return Puc_easydoc_Plugin_Info
		 */
		public function requestInfo($queryArgs = []) {
			list($pluginInfo, $result) = $this->requestMetadata('Puc_easydoc_Plugin_Info', 'request_info', $queryArgs);

			if ( $pluginInfo !== null ) {
				/** @var Puc_easydoc_Plugin_Info $pluginInfo */
		 ...
		 *
		 * @return Puc_easydoc_Update|null An instance of Plugin_Update, or NULL when no updates are available.
		 */
		public function requestUpdate() {
			...
			$update = Puc_easydoc_Plugin_Update::fromPluginInfo($pluginInfo);
		 ...
		 *
		 * @return Puc_easydoc_Plugin_Update|null
		 */
		public function getUpdate() {
			$update = parent::getUpdate();
			if ( isset($update) ) {
				/** @var Puc_easydoc_Plugin_Update $update */
		...
		protected function createDebugBarExtension() {
			// return new Puc_easydoc_DebugBar_PluginExtension($this);
		}
	}
