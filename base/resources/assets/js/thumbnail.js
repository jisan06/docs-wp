...
(function($){
    ...
    this.ThumbnailBox = Vue.extend({
        ...
        mounted: function() {
            ...
            imageEditor = EasyDoc.ImageEditorFactory.getEditor(null, {
            ...
            if (vm.hasConnectSupport) {
                window.addEventListener('message', function(event) {
                    if (event.origin.match(/^https:\/\/(.*?)\.system.ait-themes.(?:com|xyz)$/)) {
        ...
        methods: {
            ...
            changeCustom: function() {
                if (!this.changeCustomEventAdded) {
                    this.changeCustomEventAdded = true;

                    var uploader_el = kQuery('.image-uploader');
                    ...
                    uploader.uploader.bind('FileUploaded', function(up, file, result) {
                        if (result.status === 201 && typeof result.response === 'object') {
                            if (result.response.entities && result.response.entities.length) {
                                var image = result.response.entities[0];
                                window.onSelectImage(image.path);
                            }
                        }
                    });
                }

                document.querySelector('.k-js-image-uploader-container .js-choose-files').click();
            },
            openPicker: function() {
                var vm = this;
