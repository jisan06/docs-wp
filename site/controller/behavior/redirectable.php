 ...
 * @return void
 */
class ControllerBehaviorRedirectable extends Library\ControllerBehaviorAbstract
{
    ...
    public function __construct(Library\ObjectConfig $config)
    {
        ...
        $this->_redirect_schemes = Library\ObjectConfig::unbox($config->redirect_schemes);
    ...
    {
        $config->append([
            'priority' => static::PRIORITY_LOW,
            'redirect_schemes' => ['http', 'https'],
     ...
     * @return void
     */
    protected function _actionRedirect(Library\ControllerContext $context)
    {
    ...
    protected function _rewriteGoogleDocsUrl($url)
    {
        ...
        if(stripos($url_path, 'folders/') === false && $id = $this->_getGoogleId($url_path))
        {
            ...
            foreach(['presentation','document','spreadsheets'] as $type)
            {
